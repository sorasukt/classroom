...
<script>
const API_BASE='https://classroom.sorasukt.com';
...
function beginLogin(){location.href=API_BASE+'/api/auth/login'}
function logout(){location.href=API_BASE+'/api/auth/logout'}
...
function downloadCsv(){const id=document.getElementById('reportClass').value;if(!id)return toast('กรุณาเลือกห้องเรียน');const query=new URLSearchParams({classroom_id:id,from:document.getElementById('reportFrom').value,to:document.getElementById('reportTo').value});location.href=API_BASE+'/api/export.csv?'+query}
